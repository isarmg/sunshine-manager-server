...
export function DeviceRegistrationDialog({close,created}:{close():void;created(ticket:Ticket):void}){
 const{client,notify}=useAdminApplication();const[pending,setPending]=useState(false);const[failure,setFailure]=useState<{requestId?:string}|null>(null);
 ...
 try{const ticket=await client.request(`${CURRENT_API_PREFIX}/sunshine/devices`,isTicket,{method:"POST",body:JSON.stringify({name})});created(ticket);notify(t("Sunshine 实例已创建", "Sunshine instance created"));}catch(error){setFailure({requestId:errorRequestId(error)})}finally{setPending(false)}}

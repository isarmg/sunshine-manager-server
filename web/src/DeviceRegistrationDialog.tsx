...
export function TicketPanel({ticket}:{ticket:Ticket}){
 ...
 <p>{t("客户端只需填写服务器地址、此配对码，以及本机 Sunshine 地址、账号密码和 Sunshine 自带 cacert.pem 的路径；设备标识自动获取。管理服务器仍要求系统信任的 HTTPS，Sunshine 自签名证书则由客户端精确固定。服务器不接收 Sunshine 密码或证书。", "Enter the server address, this pairing code, the local Sunshine address and credentials, and the path to Sunshine's own cacert.pem. Device IDs are resolved automatically. The manager server still requires system-trusted HTTPS, while the client pins Sunshine's self-signed certificate exactly. The server receives neither the Sunshine password nor certificate.")}</p></section>;
}

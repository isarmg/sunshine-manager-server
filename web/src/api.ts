...
export function isTicket(value:unknown):value is Ticket{return record(value)&&isDevice(value.device)&&typeof value.manager_id==="string"&&typeof value.token==="string"&&/^[a-z0-9]{32}$/.test(value.token)}
export function isClientAuthorization(value:unknown):value is ClientAuthorization{return record(value)&&Object.keys(value).length===3&&typeof value.manager_id==="string"&&/^[0-9a-f-]{36}$/.test(value.manager_id)&&typeof value.device_id==="string"&&/^[0-9a-f-]{36}$/.test(value.device_id)&&typeof value.authorization_code==="string"&&(/^[a-z0-9]{32}$/.test(value.authorization_code)||/^[a-f0-9]{64}$/.test(value.authorization_code))}

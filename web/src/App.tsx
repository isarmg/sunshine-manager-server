...
function DevicesPage(){
 ...
 useEffect(()=>{const controller=new AbortController();let active=true;void client.request(CURRENT_API_PREFIX+"/sunshine/config-fields",isConfigFieldDefinitions,{signal:controller.signal}).then(value=>{if(active)setFieldDefinitions(value)}).catch(error=>{if(active)setFailure({requestId:errorRequestId(error)})});return()=>{active=false;controller.abort()}},[client]);
 ...
 {page==="instances"?<div className="sarmg-content-stack" aria-label={t("Sunshine 实例", "Sunshine instances")}>
  ...
  <section className="sarmg-content-stack"><h2>{t("实例列表", "Instance list")}</h2>{devices===null?<LoadingState>{t("正在加载设备…", "Loading devices…")}</LoadingState>:<DeviceInstances devices={devices} select={id=>{setSelected(id);window.location.hash=`details/${id}`}}/>}</section>
 </div>
 :device?<><h2>{device.name}</h2><ClientWorkspace key={device.id} device={device} fieldDefinitions={fieldDefinitions} page={page} changed={refresh} ticket={ticket?.device.id===device.id?ticket:null}/></>:devices===null?<LoadingState>{t("正在加载设备…", "Loading devices…")}</LoadingState>:<EmptyState>{t("暂无实例，请新建并注册 客户端。", "No instances yet. Create an instance and register an client.")}</EmptyState>}

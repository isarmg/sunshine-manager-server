...
function DevicesPage(){
 ...
 return <section className="sarmg-content-stack"><InstanceHeaderActions create={()=>void createDevice()} refresh={refresh} refreshing={creating}/><InstancePageNavigation page={page} detailsDisabled={!device} navigate={value=>{window.location.hash=device&&value!=="instances"?`${value}/${device.id}`:value}}/><h1 className="sarmg-visually-hidden">{t("Sunshine 设备管理", "Sunshine device management")}</h1>
...
export default createSarmgAdminApplication({product:{name:"Sunshine Manager"},client:adminApi,navigation:[],loginLandingHref:"#instances",routes:<DevicesPage/>});

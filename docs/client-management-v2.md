...
Manager 在数据库确认设备凭据无效时，WebSocket 握手返回 401 与 `X-Sarmg-Error-Code: unauthorized`；入口代理须透传此响应头，Client 才能立即停止重连并等待重新配对。缺失或格式错误的 `Authorization`、入口代理产生的无标识 401、入口校验 403 及临时服务错误按退避策略重连；若代理暂时丢失认证头，恢复透传后连接可自动恢复。

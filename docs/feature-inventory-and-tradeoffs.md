...
本文只描述 `0.10.7` 当前实现。事实源依次为 `src/http.rs::router()`、`protocol/`、`schema/product.sql`、
...
| 软件/API | 0.10.7，管理 API `/api/v2` | 不注册旧 API alias |
...
- 产品不内置 migration、backup、restore 或 key rotation。`sarmg-upgrade` 当前也未声明支持 0.10.7；

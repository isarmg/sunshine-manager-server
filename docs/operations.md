...
```text
/opt/isarmg/sunshine-manager/releases/0.11.6/  root-owned read-only release
...
```text
/opt/isarmg/sunshine-manager/releases/0.11.6/bin/sunshine-manager \
  serve-release --root /opt/isarmg/sunshine-manager/releases/0.11.6
```
...
从干净且 annotated `v0.11.6` 精确指向 HEAD 的 checkout：

```bash
python3 scripts/package-release.py /absolute/release-output
```

当前 Server Rust 固定 Foundation 0.8.8 / `946bbb5f4bf3e06d421b6f463bd637b83617a5be`；八个 Web 包
...
```bash
tar -xzf sunshine-manager-0.11.6-x86_64-unknown-linux-gnu.tar.gz \
  -C /opt/isarmg/sunshine-manager/releases
/opt/isarmg/sunshine-manager/releases/0.11.6/bin/sunshine-manager \
  verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.6
```
...
sunshine-manager verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.6
...
[简化配对](https://github.com/isarmg/sunshine-manager-server/blob/v0.11.6/docs/simple-pairing.md)。
...
backup/verify/restore，并未声明支持本仓当前 0.11.6。不能因依赖更新或构建通过推定备份可跨版本恢复；

...
```text
/opt/isarmg/sunshine-manager/releases/0.11.8/  root-owned read-only release
...
```text
/opt/isarmg/sunshine-manager/releases/0.11.8/bin/sunshine-manager \
  serve-release --root /opt/isarmg/sunshine-manager/releases/0.11.8
```
...
从干净且 annotated `v0.11.8` 精确指向 HEAD 的 checkout：

```bash
python3 scripts/package-release.py /absolute/release-output
```

当前 Server Rust 固定 Foundation 0.9.0 / `b146afefb1e864de78d5f6ac43cab7d1a4e02fc7`；八个 Web 包
...
```bash
tar -xzf sunshine-manager-0.11.8-x86_64-unknown-linux-gnu.tar.gz \
  -C /opt/isarmg/sunshine-manager/releases
/opt/isarmg/sunshine-manager/releases/0.11.8/bin/sunshine-manager \
  verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.8
```
...
sunshine-manager verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.8
...
[简化配对](https://github.com/isarmg/sunshine-manager-server/blob/v0.11.8/docs/simple-pairing.md)。
...
backup/verify/restore，并未声明支持本仓当前 0.11.8。不能因依赖更新或构建通过推定备份可跨版本恢复；

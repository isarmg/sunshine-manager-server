...
```text
/opt/isarmg/sunshine-manager/releases/0.11.11/  root-owned read-only release
...
从干净且 annotated `v0.11.11` 精确指向 HEAD 的 checkout：
...
```bash
tar -xzf sunshine-manager-0.11.11-x86_64-unknown-linux-gnu.tar.gz \
  -C /opt/isarmg/sunshine-manager/releases
/opt/isarmg/sunshine-manager/releases/0.11.11/bin/sunshine-manager \
  verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.11
```
...
sunshine-manager verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.11
...
[简化配对](https://github.com/isarmg/sunshine-manager-server/blob/v0.11.11/docs/simple-pairing.md)。

## 7. 当前连续性限制与外部升级边界

Sunshine Manager 产品仓没有 backup、restore、Schema conversion、key rotation 或 re-encryption 命令。
当前 Server 软件版本为 0.11.11，数据库身份独立保持为 Sunshine Manager `0.10.1`、Schema revision 7、

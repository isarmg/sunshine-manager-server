...
```text
/opt/isarmg/sunshine-manager/releases/0.11.3/  root-owned read-only release
...
从干净且 annotated `v0.11.3` 精确指向 HEAD 的 checkout：
...
```bash
tar -xzf sunshine-manager-0.11.3-x86_64-unknown-linux-gnu.tar.gz \
  -C /opt/isarmg/sunshine-manager/releases
/opt/isarmg/sunshine-manager/releases/0.11.3/bin/sunshine-manager \
  verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.3
```
...
sunshine-manager verify-release --root /opt/isarmg/sunshine-manager/releases/0.11.3
